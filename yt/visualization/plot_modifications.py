...
class MapFeaturesCallback(PlotCallback):
    """
    Annotate map features.  This is done through a second call to add map
    features to the radar cartopy plot.

    Parameters
    ----------
    tile : instance
        Cartopy image tile instance to be used in the plot. Example:
        cartopy.io.img_tiles.StamenTerrain()
    embelish : bool
        When the second image is overlaid, it will have this level of alpha
        transparency. Default is 1.0 (fully-opaque).
    resolution : '10m', '50m', '110m'
        Resolution of NaturalEarthFeatures to use. See Cartopy documentation
        for details.
    zorder : float
        If a tile is applied, zorder will set the order of when the tile is
        plotted. Lower numbers are plotted first. Default is 0.0.
    lat_lines, lon_lines : array or None
        Locations at which to draw latitude and longitude lines.
        None will not draw latitude and longitude lines.

    """
    _type_name = "map_features"
    _supported_geometries = ("cartesian")
    def __init__(self, tile=None, embelish=False, resolution="110m",
                 zorder=0.0, lat_lines=None, lon_lines=None):
        PlotCallback.__init__(self)
        self.tile = tile
        self.embelish = embelish
    ...
    def __call__(self, plot):
        ...
        if self.tile is not None:
            plot._axes.add_image(self.tile, 8, zorder=self.zorder)

        if self.embelish is True:
            state_provinces = cartopy.feature.NaturalEarthFeature(
                category="cultural",
                name="admin_1_states_provinces_lines",
                scale=self.resolution,
                facecolor="None")
            plot._axes.add_feature(state_provinces, edgecolor="gray")
            plot._axes.coastlines(resolution=self.resolution)

        if lat_lines and lon_lines is not None:
            if plot._axes.projection in [cartopy.crs.PlateCarree(),
                                         cartopy.crs.Mercator()]:

                plot._axes.set_xticks(
                    self.lon_lines, crs=cartopy.crs.PlateCarree())
                plot._axes.set_yticks(
                    self.lat_lines, crs=cartopy.crs.PlateCarree())
                lon_formatter = LongitudeFormatter(zero_direction_label=True)
                lat_formatter = LatitudeFormatter()
                plot._axes.xaxis.set_major_formatter(lon_formatter)
                plot._axes.yaxis.set_major_formatter(lat_formatter)
                plot._axes.gridlines(xlocs=lon_lines, ylocs=lat_lines)
            elif isinstance(ax.projection, cartopy.crs.LambertConformal):
                plot.figure.canvas.draw()
                plot_axes.gridlines(xlocs=lon_lines, ylocs=lat_lines)

                # Label the end-points of the gridlines using the custom
                # tick makers:
                plot._axes.xaxis.set_major_formatter(
                    cartopy.mpl.gridliner.LONGITUDE_FORMATTER)
                plot._axes.yaxis.set_major_formatter(
                    cartopy.mpl.gridliner.LATITUDE_FORMATTER)
                if _LAMBERT_GRIDLINES:
                    lambert_xticks(plot._axes, self.lon_lines)
                    lambert_yticks(plot._axes, self.lat_lines)
            else:
                plot._axes.gridlines(
                    xlocs=self.lon_lines, ylocs=self.lat_lines)
    ...
    def lambert_xticks(ax, ticks):
        """ Draw ticks on the bottom x-axis of a Lambert Conformal
        projection. """
        def te(xy):
            return xy[0]
    ...
    def lambert_yticks(ax, ticks):
        """ Draw ticks on the left y-axis of a Lambert Conformal
        projection. """
        def te(xy):
            return xy[1]
    ...
    def _lambert_ticks(ax, ticks, tick_location, line_constructor,
                       tick_extractor):
        """ Get the tick locations and labels for a Lambert Conformal
        projection. """
